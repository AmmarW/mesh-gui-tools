...
void ObjParser::generateVolumeMesh(Mesh& mesh) {
    ...
    size_t n = mesh.vertices.size(); // Use size_t here.
    int refinement_factor = 1; // This can remain int if it makes sense in your context.

    for (size_t i = 0; i < n - 3; i++) {
        for (int r = 0; r < refinement_factor; ++r) { // refinement_factor is still int.
            size_t j = (i + 1 + r) % (n - 1);
            size_t k = (i + 2 + r) % (n - 1);
            size_t l = (i + 3 + r) % (n - 1);
            // If Mesh::Tetrahedron requires ints, cast the indices appropriately.
            tetrahedralMesh.insert({static_cast<int>(i), static_cast<int>(j), static_cast<int>(k), static_cast<int>(l)});
        }
    }
