...
// Basic 3D vertex structure.
struct Vertex {
    double x, y, z;
};

// A face element that may include vertex, texture, and normal indices.
struct FaceElement {
...
// A face is a collection of face elements.
struct Face {
    std::vector<FaceElement> elements;
};

// The Mesh class holds the entire mesh data.
class Mesh {
public:
